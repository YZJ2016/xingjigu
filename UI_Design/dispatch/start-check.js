...
function bindEvents() {
  ...
  $("#batchPassBtn").addEventListener("click", () => {
    ...
    if (!targets.length) {
      showToast("没有可记录准入放行的开工任务");
      return;
    }
    targets.forEach((item) => {
      item.status = "已放行";
      recordLog(item.id, "已批量记录开工准入放行结果，等待模拟扫码枪开工回执");
      syncLedgerCheck(item, { status: "已放行" }, "已批量记录开工准入放行结果，等待模拟扫码枪开工回执");
    });
    state.activeCheckId = targets[0].id;
    saveState();
    renderAll();
    showToast(`已记录 ${targets.length} 个开工准入放行结果`);
  });
  ...
  $("#releaseBtn").addEventListener("click", () => {
    ...
    updateCheck(item.id, { status: "已放行" }, "已记录开工准入放行结果，等待模拟扫码枪开工回执");
  });
