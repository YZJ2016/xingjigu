...
function renderFrameMenu() {
  $("#sopModuleMenu").innerHTML = modules.map((module) => {
    ...
    const submenu = module.items.map((item, index) => {
      const active = module.id === "dispatch" && item === "工艺文件与作业指导" ? " class=\"is-active\"" : "";
...
function syncLedgerSop(item, message) {
  ...
  if (!row) return;
  ledger.appendRecord?.(row.dispatchId, message, { owner: item.owner || "工艺员", source: message.includes("模拟") ? "MES 工艺文件与作业指导接收模拟回执" : "MES 工艺文件与作业指导" });
}
