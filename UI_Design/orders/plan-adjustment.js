...
function approveAdjustment(orderId) {
  ...
  updateOrder(order.id, { schedule: "已确认", status: order.status === "待排程" ? "已排程" : order.status, batchPlan: getAfterBatch(order, adjustment), risk: order.risk === "交期" ? "正常" : order.risk }, "调整草案审批通过，已生成新排程版本并等待派工重发校验");
}
...
function bindEvents() {
  ...
  $("#splitBatchBtn").addEventListener("click", () => {
    ...
    recordIntegration(order.id, "已维护为拆批调整草案");
    saveState();
    renderAll();
    showToast("已维护为拆批调整草案");
  });
...
function shiftActiveAdjustment(days) {
  ...
  recordIntegration(order.id, days < 0 ? "已生成提前 1 天调整草案" : "已生成后移 1 天调整草案");
  saveState();
  renderAll();
  showToast(days < 0 ? "已生成提前 1 天调整草案" : "已生成后移 1 天调整草案");
}
