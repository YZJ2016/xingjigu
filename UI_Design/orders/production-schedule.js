...
function renderVersionAndRisks() {
  ...
  $("#versionList").innerHTML = [
    ...
    ["发布边界", "生成受控排程版本后才允许推送派工、备料、检验和条码准备指令", plan.release],
...
function confirmPlan(order) {
  ...
  updateOrder(order.id, { schedule: "已确认", status: nextStatus, batchPlan: getPlan(order).batchPlan, scheduleVersion: getPlan(order).version }, "已生成受控排程版本，可推送派工与备料准备");
}
...
function bindEvents() {
  ...
  $("#autoPlanBtn").addEventListener("click", () => {
    ...
    targets.forEach((order) => {
      ...
      schedulePlans[order.id] = buildPlan(order);
      recordIntegration(order.id, order.schedule === "已确认" ? "已生成自动排程版本" : "已生成排程建议，等待约束确认");
    });
    ...
    syncPageStore();
    renderAll();
    showToast(`已生成 ${targets.length} 个排程建议/版本草案`);
  });
  ...
  $("#splitBatchBtn").addEventListener("click", () => {
    const order = getActiveOrder();
    updateOrder(order.id, { schedule: "待调整", batchPlan: getPlan(order).batchPlan }, "已生成按物料和瓶颈资源拆批的调整草案");
  });
  $("#resolveConstraintBtn").addEventListener("click", () => {
    const order = getActiveOrder();
    updateOrder(order.id, { risk: "正常", kit: "齐套", materialGap: "齐套", schedule: "已确认", status: "已排程" }, "排程约束已确认，计划版本已锁定");
  });
  ...
  $("#releaseDispatchBtn").addEventListener("click", () => {
    const order = getActiveOrder();
    if (order.schedule !== "已确认") {
      showToast("请先生成排程版本再推送派工准备指令");
      return;
    }
    updateOrder(order.id, { status: order.status === "已排程" ? "已下达" : order.status }, "已推送派工、备料和检验准备指令并等待回执");
  });
