...
function renderFrameMenu() {
  $("#stationModuleMenu").innerHTML = modules.map((module) => {
    ...
    const submenu = module.items.map((item, index) => {
      const active = module.id === "station" && item === "工位身份回执" ? " class=\"is-active\"" : "";
...
function bindEvents() {
  ...
  $("#loginBtn").addEventListener("click", () => loginEmployee(getActiveEmployee(), "已同步模拟现场身份绑定回执"));
